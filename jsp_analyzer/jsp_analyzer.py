...
class JspPreprocessor:
    ...
    def preprocess(self, content: str) -> str:
        ...
        #    ※ スクリプトレット変換の前に実行すること
        content = self.re_other_directive.sub("", content)
...
class JspAnalyzer:
    ...
    def _extract_attr_info(self, tag: Tag, parts: List[str], tag_name: str) -> None:
        ...
        for attr, value in tag.attrs.items():
            ...
            if attr.startswith("on"):
                parts.append(f"{ICON_EVENT}{attr}='{str_value}'")
            # 属性内のEL式
            elif "${" in str_value:
                parts.append(f"{ICON_DATA} [Attr EL]: {attr}='{str_value}'")
            # href属性（aタグ）
    ...
    def _process_text(
        ...
        if el_matches:
            lines.append(f"{indent}{ICON_DATA} [Data]: {' '.join(el_matches)}")
...
def main() -> None:
    ...
    parser.add_argument("input_dir", type=str, help="解析対象のディレクトリパス")
    parser.add_argument(
        "--output-dir",
        type=str,
        default=None,
